...
uint8_t program_num = 52;

struct OBJ{
	float px, py, vx, vy, hd, dt;
...
struct point {
	float x, y;
	char pnum;	//for 8bit comm.
} mdata;

// ============================================================================================
// for SWARM DYNAMICS 1
// ============================================================================================
...
{      
	...
	//	}
		...
		//if(servo_on){
			servo_motor_control();	//??? I do not know how to make free
		//	servo_on=0;
		//}
		...
		//if(calculation_on){
//fprintf_P(&usart_stream, PSTR("CPU: %d\r\n"), program_num);
			switch(program_num){
		...
		if(jiffies>end_time){
			LED_PORT.OUT = LED_USR_1_PIN_bm;		// toggle yellow LED
		}

		while(jiffies<end_time);

		if(jiffies>1000) jiffies=0;
	}
	return 0;
}
...
void swarm_calculation1()
{
	...
	forcex=0; forcey=0;
	for(i=0;i<6;i++){
		if(mchip.open[i]!=0) swarm_interaction1(i);
	}
...
{
	init_common();
	if(rand()%2) mchip.px=1.0; else mchip.px=2.0;
	mchip.py=0; 
	mchip.hd=0;
}
